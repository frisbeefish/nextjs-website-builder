...
import { useGlobalState } from "@/app/_client/contexts/global-state";
...
import { getEditorForPage, createNewPageOfType } from "@/app/_client/utils";
...
export default function Home() {
  ...
  */
  ...
  useEffect(() => {
    apiGetWebsiteJSON()
      .then(function (websiteJSON) {
        //alert("got website json");
        //alert(JSON.stringify(websiteJSON, null, 3));
        const pages = websiteJSON.pages.map(function (page) {
          const pageTemplateObject = {
            [PAGE_TYPES.SINGLE_IMAGE_PAGE]: SINGLE_IMAGE_PAGE_TEMPLATE.clone(),
  ...
  /*
  useEffect(() => {
    setPageEditor(selectedPage ? getEditorForPage(selectedPage) : null);
  }, [selectedPage]);
  */

  /*
  PageEditor = getEditorForPage(selectedPage);
  */
  ...
  function createNewPage(pageType) {
    ...
    const page = createNewPageOfType(pageType);
    page.name = pageType + new Date().getTime();
    setPages([...pages, page]);
  ...
  showUnsavedDataModal
    */
    ...
    //alert(JSON.stringify(page, null, 3));
    setSelectedPage(page);
  }
  ...
  */
  ...
  const selectedPageOptions = pages.map((page, index) => {
    return (
  ...
  */
  ...
  const PageEditor = getEditorForPage(selectedPage);
  ...
  return (
    <Box backgroundColor="#f0f4f4" position="relative" pt={0}>
      ...
      <Box marginLeft="auto" marginRight="auto" minHeight="100vh" p={4}>
        <Flex
